function [L, type] = dubinLRL(s, g, r_turn_min)
    ...
    % Verificar si es posible el camino LRL.En rigor se debe verificar que
    % D > 4 * r_turn_min, pero en el límite puede fallar
    if D > 4 * r_turn_min * 0.95
        warning('La trayectoria LRL no es válida para los puntos dados.');
        L = [-1 -1 -1];
        return
    end
    ...
    theta1_arc = checkAngles(theta1_end - theta1_start, 'L');
    ...
    theta2_arc = checkAngles(theta2_end - theta2_start, 'R');
    ...
    theta3_arc = checkAngles(theta3_end - theta3_start, 'L');
