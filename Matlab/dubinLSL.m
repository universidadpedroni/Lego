function [L, type] = dubinLSL(s, g, r_turn_min)
    ...
    if D < 2 * r_turn_min
        warning('La trayectoria LSL no es válida para los puntos dados.');
        L = [-1 -1 -1];
        return
    end
    ...
    if isempty(tangents)
        warning('No se encontraron tangentes válidas para el tipo LSL.');
        L = [-1 -1 -1];
        return
    end
