function [L, type] = dubinLSR(s, g, r_turn_min)
    ...
    if D < 2 * r_turn_min
        warning('La trayectoria LSR no es válida para los puntos dados.');
        L = [-1 -1 -1];
        return
    end
    ...
    if isempty(tangents)
        warning('No se encontraron tangentes válidas para el tipo LSR.');
        L = [-1 -1 -1];
        return
    end
    ...
    theta1_arc = checkAngles(theta1_arc, 'L'); % Corregir el ángulo para arco izquierdo
    ...
    theta2_arc = checkAngles(theta2_arc, 'R'); % Corregir el ángulo para arco derecho
